...
> **How the guidance files fit together** — Sections 1–9 below are the hand-written,
> project-specific source of truth (this file is what Codex and Zed read). The
> `<laravel-boost-guidelines>` block at the bottom is **generated by `php artisan boost:install`**
> — do not hand-edit it; it is the same content Boost writes to `CLAUDE.md` for Claude Code, so
> each tool's entry file is self-contained. Path-scoped detail lives in `.ai/rules/*` (all agents
> read those). When a rule here conflicts with a generated Boost guideline, the project rule and
> `.ai/rules` win — see §7 and `.ai/rules/testing.md` (e.g. Pest over the older PHPUnit guideline).
...
10. **Work as a senior collaborator, not a silent executor.** This base is under active
    development. Say plainly what you changed and why, recommend the better path when you see one
    (raising standards like the PHPStan level deliberately, not all at once without agreement), and
    keep the human in the loop on judgment calls. Cheap checks (Pint, Larastan) run as you go; the
    **full test suite is batched to the end of a coherent change, not re-run after every small
    edit** — see §4.
...
**While iterating (cheap, no database — run freely)**

1. `vendor/bin/pint --dirty` and `vendor/bin/phpstan analyse` are fast and touch no database. Run
   them as you go to keep the diff formatted and typed.

**At the end of a coherent chunk (batch the expensive checks — do NOT repeat them per edit)**

2. Run the focused tests for the slice once (validation, authorization, persistence, response/view)
   — not after every one-line edit. Then the guards on the diff: `laravel-guard`, then
   `clean-code-guard`; `test-guard` if tests changed; `docs-guard` if public behavior or docs changed.
3. Run the full handoff gate (§5) before handoff.
4. Report residual risks honestly. Batching verification never means skipping a check that was
   needed — it means grouping the slow ones (the full suite) at the end instead of the middle.
